...
jQuery(async () => {
    ...
    async function showEditView(presetName, selectedEntryIdentifier = null) {
        ...
        const paramsEditorContainer = $('#pmm-prompt-editor-container').empty();

        try {
            const presetData =
                await PresetManagerAPI.getPresetByName(presetName);
            if (!presetData) {
    ...
    async function renderEntriesForTransfer() {
        ...
        try {
            const presetData =
                await PresetManagerAPI.getPresetByName(sourcePresetName);
    ...
    async function renderEntriesForDeletion() {
        ...
        try {
            const presetData =
                await PresetManagerAPI.getPresetByName(selectedPresetName);
            entryList.empty();
        ...
        try {
            ...
            **你的输出 (纯文本):**`;
            ...
            if (!window.TavernHelper || !window.TavernHelper.generateRaw) {
                throw new Error('TavernHelper 或其 generateRaw 函数不可用。请确保 JS-Slash-Runner 插件已启用。');
            }
            const rawAiResponse = await window.TavernHelper.generateRaw({
    ...
    async function handleAiCreateEntry() {
        ...
        try {
            ...
            **核心规则:**
            ...
            // 获取目标预设的当前数据
            const targetPreset =
                await PresetManagerAPI.getPresetByName(targetPresetName);
            if (!targetPreset) {
    ...
    function makeButtonDraggable($button) {
        ...
        const onDragMove = (e) => {
            if (isDragging) {
                ...
                // 仅当移动超过一个小阈值时才标记为拖动
                if (
                    Math.abs(pageX - startX) > 5 ||
                    Math.abs(pageY - startY) > 5
                ) {
                    wasDragged = true;
                }
    ...
    async function initializeExtension() {
        ...
        $('#preset-manager-momo-check-update-btn').on('click touchend', (e) => {
            // 使用完整的ID
            e.preventDefault();
            e.stopPropagation();
        ...
        // -- 视图切换按钮
        $('#pmm-goto-delete-btn').on('click touchend', (e) => {
            e.preventDefault();
            showDeleteView();
        });
        $('#pmm-goto-transfer-btn').on('click touchend', (e) => {
            e.preventDefault();
            showTransferView();
        });
        $('#pmm-goto-ai-create-btn').on('click touchend', (e) => {
            e.preventDefault();
            showAiCreateView();
        }); // 绑定新按钮
        ...
        // -- AI 创建视图的事件
        $('#pmm-ai-create-btn').on('click touchend', (e) => {
            e.preventDefault();
            handleAiCreateEntry();
        });
        ...
        $('#pmm-save-entry-changes-btn').on('click touchend', (e) => {
            e.preventDefault();
            handleSaveEntryChanges();
        });
        $('#pmm-submit-entry-ai-btn').on('click touchend', (e) => {
            e.preventDefault();
            handleEntryAiAssist();
        });
        $('#pmm-save-ai-result-btn').on('click touchend', (e) => {
            e.preventDefault();
            handleSaveAiResult();
        });
        $('#pmm-clear-ai-result-btn').on('click touchend', (e) => {
            e.preventDefault();
            handleClearAiResult();
        });
        $('#pmm-save-manual-changes-btn').on('click touchend', (e) => {
            e.preventDefault();
            handleSaveManualChanges();
        });
        ...
        if (transferBtn.length) {
            ...
            transferBtn.on('click touchend', (e) => {
                e.preventDefault();
                handleTransferEntries();
            });
        } else {
            console.error(
