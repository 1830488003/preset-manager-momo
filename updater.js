...
(async function () {
    ...
    const {
        getRequestHeaders,
        extension_api: { POPUP_TYPE, callGenericPopup, t },
    ...
    const CHANGELOG_FILE_PATH =
        'preset-manager-momo/preset-manager-momo项目文档.txt'; // 指向项目文档
    ...
    async function fetchRawFileFromGitHub(filePath) {
        ...
        try {
            ...
            console.error(
                `[${EXTENSION_NAME}] 获取 GitHub 文件内容时出错:`,
                error,
            );
            throw error;
        }
    ...
    async function getLocalFileContent(filePath) {
        try {
            ...
            console.error(
                `[${EXTENSION_NAME}] 读取本地文件 ${filePath} 失败:`,
                error,
            );
            throw error;
        }
    ...
    async function checkForUpdates() {
        ...
        try {
            const remoteManifestContent =
                await fetchRawFileFromGitHub(VERSION_FILE_PATH);
            ...
            const localManifestContent =
                await getLocalFileContent(localManifestPath);
            ...
            if (compareVersions(remoteVersion, localVersion) > 0) {
                ...
                checkBtn
                    .text('立即更新')
                    .off('click touchend')
                    .on('click touchend', handleUpdateButtonClick);
            } else {
    ...
    async function handleUpdateButtonClick(event) {
        ...
        btn.prop('disabled', true).html(
            '<i class="fas fa-spinner fa-spin"></i> 获取日志...',
        );

        try {
            if (!changelogContent) {
                const fullLog =
                    await fetchRawFileFromGitHub(CHANGELOG_FILE_PATH);
                // 简化处理：直接显示最新版本的日志
                const versionHeader = `### **版本 ${remoteVersion}`;
                const logStartIndex = fullLog.indexOf(versionHeader);
                if (logStartIndex !== -1) {
                    const nextVersionIndex = fullLog.indexOf(
                        '### **版本',
                        logStartIndex + 1,
                    );
                    changelogContent =
            ...
            const formattedLog = changelogContent
                .replace(/###\s*(.*)/g, '<h3>$1</h3>')
                .replace(/\n/g, '<br>');
    ...
    async function executeUpdate(btn) {
        ...
        try {
            ...
            toastr.error(`更新失败: ${error.message}`, '错误', {
                timeOut: 10000,
            });
